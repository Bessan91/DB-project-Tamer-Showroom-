...
public class Bessan {
	...
	public static void showDashboard(Stage primaryStage)
		...
		hboxDashboard.getChildren().addAll(title);
...
//		----------------------butoon number of employee 
		...
		button_Count_employee.setOnAction(e -> {

			Hajar.showMainPage(new Stage());
		});
...
//		ImageView Emp = new ImageView(new Image("Employee.png"));
...
//		import3_table.setPadding(new Insets(30, 0, 0, 120));
		...
		Scene scene = new Scene(borderPane, 1260, 640);
	...
	private static void dashBordContent_left(BorderPane pp, Stage s) {
		...
		vbox_Dashbord.setMaxHeight(1800);
...
//	=====================================Start available in store  (Storge )========
		Button button_Storge = new Button("Storage");
...
//	 ==================================Manitanince =================
		...
		button_Logout.setOnAction(e->{
				...
				Scene scene;
				try {
					scene = new Scene(Hajar.Sign_In(s), 595, 332);
					s.setTitle("Sign In");
					s.setScene(scene);
					s.show();
				} catch (FileNotFoundException e1) {
...
//				New_sell(s, pp);
		...
		button_Storge.setOnAction(e -> {

			s.close();
			Storage page;
			try {
				page = new Storage();
				Scene car = new Scene(page.bp, 950, 600);

				page.storage.setScene(car);
				page.storage.setTitle("Storage");
				page.storage.show();
			} catch (FileNotFoundException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
		...
		button_maintenance.setOnAction(e -> {
			s.close();
			Maintenance page = new Maintenance();
			Scene scene;
			try {
				scene = new Scene(page.UserInterface(), 1100, 635);
				page.MaintenanceStage.setScene(scene);
				page.MaintenanceStage.setTitle("Maintenance");
				page.MaintenanceStage.show();
			} catch (FileNotFoundException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
		});
		button_Report.setOnAction(e -> {
			s.close();
			Reports page;
			try {
				page = new Reports();
				Scene scene = new Scene(page.bp, 1000, 635);
				page.ReportsStage.setScene(scene);
				page.ReportsStage.show();
			} catch (FileNotFoundException e1) {
				// TODO Auto-generated catch block
				e1.printStackTrace();
			}
	...
	public static void New_sell2(Stage s, BorderPane pp)
...
//					});
					ConfirmCheck.setOnAction(comfirm -> {
						newSellStage.close();
						Paying_Off p;
						try {
							p = new Paying_Off();
							Scene scene = new Scene(p.bp, 1100, 635);
							p.pay.setTitle("Paying Off");
							p.pay.setScene(scene);
							p.pay.show();
						} catch (FileNotFoundException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
...
//		ImageView view = new ImageView(new Image("sell_Bckground.png"));
	...
	static Node CustomerInformation(Stage stageDashboard) throws SQLException {
...
//	    int count2 =0 ;
		...
		while (rs.next()) {
			...
			data.add(new Customer((rs.getString("Customer_id")), rs.getString("FullName"), rs.getString("phone")));
		}
		rs.close();
		stmt.close();
		// con.close();
		...
		return createDataTableCustomerSummary(CustomerdataList);
	}
...
//			Mainten_id = rs.getString("Mainten_id");
			...
			LocalDate Mainten_date = rs.getDate("Mainten_date").toLocalDate();
...
//	carIdCol.setMaxWidth(150);
	...
	static Node DetailsCustomerInSellSection(Label name, Label phone, Label CustomerId)
...
//		hboxCustomerInfo.setStyle("-fx-background-color: black;");
	...
	private static void customerInformationSection(Stage stageDashboard) throws ClassNotFoundException, SQLException, FileNotFoundException {
		...
		Delete.setOnAction(delete -> {
			try {
				...
				while (rsCustomer.next()) {
					data.add(new Customer(rsCustomer.getString("Customer_id"), rsCustomer.getString("FullName"),
